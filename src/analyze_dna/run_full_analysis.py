...
def generate_exhaustive_genetic_report(results: dict, subject_name: str | None = None) -> str:
    """Generate the exhaustive lifestyle/health genetic report and return as string."""
...
def generate_disease_risk_report(
    findings: dict, stats: dict, genome_count: int, subject_name: str | None = None
) -> str:
    """Generate the exhaustive disease risk report and return as string."""
...
def generate_actionable_protocol(health_results: dict, disease_findings: dict, subject_name: str | None = None) -> str:
...
def run_full_analysis(genome_path: Path, subject_name: str | None, output_dir: Path | None):
    """Run the complete genetic analysis pipeline."""
    import click

    print_header("FULL GENETIC HEALTH ANALYSIS")
    click.echo(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", err=True)

    if not genome_path.exists():
        click.echo(f"\nERROR: Genome file not found: {genome_path}", err=True)
        click.echo("Please provide a valid 23andMe genome file.", err=True)
        sys.exit(1)
    ...
    # Save intermediate results JSON (skip in stdout mode)
    if output_dir:
        results_json = {
        ...
        intermediate_path = output_dir / "comprehensive_results.json"
        intermediate_path.write_text(json.dumps(results_json, indent=2), encoding="utf-8")

    # Generate exhaustive genetic report
    genetic_report = generate_exhaustive_genetic_report(health_results, subject_name)
    ...
    disease_report = None
    if disease_findings:
        disease_report = generate_disease_risk_report(
            disease_findings, disease_stats, len(genome_by_rsid), subject_name
        )

    # Generate actionable protocol
    protocol_report = generate_actionable_protocol(health_results, disease_findings, subject_name)

    # Write reports
    if output_dir is None:
        # Stdout mode: concatenate all reports
        reports = [genetic_report]
        if disease_report:
            reports.append(disease_report)
        reports.append(protocol_report)
        combined = "\n\n---\n\n<!-- REPORT SEPARATOR -->\n\n---\n\n".join(reports)
        click.echo(combined)
    else:
        # File mode: write individual files
        (output_dir / "EXHAUSTIVE_GENETIC_REPORT.md").write_text(genetic_report, encoding="utf-8")
        click.echo(f"  Written: {output_dir / 'EXHAUSTIVE_GENETIC_REPORT.md'}", err=True)

        if disease_report:
            (output_dir / "EXHAUSTIVE_DISEASE_RISK_REPORT.md").write_text(disease_report, encoding="utf-8")
            click.echo(f"  Written: {output_dir / 'EXHAUSTIVE_DISEASE_RISK_REPORT.md'}", err=True)

        (output_dir / "ACTIONABLE_HEALTH_PROTOCOL_V3.md").write_text(protocol_report, encoding="utf-8")
        click.echo(f"  Written: {output_dir / 'ACTIONABLE_HEALTH_PROTOCOL_V3.md'}", err=True)
    ...
    if disease_findings:
        ...
        click.echo("\n  2. EXHAUSTIVE_DISEASE_RISK_REPORT.md", err=True)
        click.echo(f"     - {total_disease} total clinical findings", err=True)
        click.echo(f"     - {len(disease_findings['pathogenic'])} pathogenic variants", err=True)
        click.echo(f"     - {len(disease_findings['likely_pathogenic'])} likely pathogenic", err=True)
        click.echo(f"     - {len(disease_findings['risk_factor'])} risk factors", err=True)

    click.echo("\n  3. ACTIONABLE_HEALTH_PROTOCOL_V3.md", err=True)
    click.echo("     - Comprehensive protocol (lifestyle + disease risk + carrier status)", err=True)

    click.echo(f"\nFinished: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", err=True)
...
# uv run analyze-dna full-analysis <genome> --output <dir> [--name "Name"]
